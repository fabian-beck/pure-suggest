      ...
      >
        ...
        <span class="is-hidden-touch ml-2">
          <span
            v-html="displayText"
            :class="{
              'has-text-black': sessionStore.filter.hasActiveFilters(),
              'has-text-grey': !sessionStore.filter.hasActiveFilters()
            }"
          ></span>
        ...
        <v-row dense>
          <v-col cols="12" class="py-0">
            ...
            <div :class="interfaceStore.isMobile ? 'd-block' : 'd-flex align-center'">
              <div
                :class="
                  interfaceStore.isMobile ? 'd-flex align-center mb-2' : 'd-flex align-center'
                "
              >
                <v-switch
                  ref="filterSwitch"
                  v-model="sessionStore.filter.isActive"
                  label="Apply filters"
                  density="compact"
                  color="grey-darken-1"
                  hide-details
                ...
                ></v-switch>
                <span
                  class="text-body-2 mr-3 ml-3"
                  style="line-height: 24px; display: inline-flex; align-items: center"
                  :class="{ 'text-disabled': !sessionStore.filter.isActive }"
                  >to</span
                >
              </div>
              <div
                class="d-flex"
                :class="[
                  interfaceStore.isMobile ? 'ml-4' : '',
                  { 'opacity-50': !sessionStore.filter.isActive }
                ]"
              >
                <v-checkbox
                  v-model="sessionStore.filter.applyToSelected"
                  label="selected"
                  density="compact"
                  hide-details
                  class="mr-4 primary-checkbox"
                  :disabled="!sessionStore.filter.isActive"
                ></v-checkbox>
                <v-checkbox
                  v-model="sessionStore.filter.applyToSuggested"
                  label="suggested"
                  density="compact"
                  hide-details
                  class="info-checkbox"
