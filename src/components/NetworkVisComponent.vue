    ...
    <div class="box has-background-grey">
      <div class="level">
        <div class="level-left has-text-white">
          ...
          <div
            class="has-text-danger has-background-danger-light p-1"
            v-if="errorMessage"
          >
            {{ errorMessage }}
          </div>
          ...
          <v-menu :close-on-content-click="false">
            ...
            <v-list>
              <v-list-item prepend-icon="mdi-water-plus-outline">
                <v-list-item-title
                  >Number of <b>suggested</b> shown</v-list-item-title
                >
                <v-slider
                  v-model="suggestedNumberFactor"
                  :max="1"
                  :min="0.1"
                  step="0.05"
              ...
              <v-list-item prepend-icon="mdi-account">
                <v-list-item-title
                  >Number of <b>authors</b> shown</v-list-item-title
                >
                <v-slider
                  v-model="authorNumberFactor"
                  :max="2"
                  :min="0.1"
                  step="0.1"
                  @update:modelValue="plot(true)"
...
export default {
  ...
  methods: {
    plot: function (restart) {
          ...
          .force(
            ...
            d3
              ...
              .distance((d) => {
                switch (d.type) {
                  case "citation":
                    return that.isNetworkClusters && d.internal
                      ? 1500 / that.sessionStore.selectedPublications.length
                      : 10;
                  case "keyword":
                    return 0;
                  case "author":
            ...
            d3
              ...
              .strength(
                Math.min(
                  -200,
                  -100 *
                    Math.sqrt(that.sessionStore.selectedPublications.length)
                )
              )
          )
          .force(
            ...
            d3
              ...
              .x((d) => {
                ...
                switch (d.type) {
                  ...
                  default:
                    return this.yearX(
                      (d.author.yearMax + d.author.yearMin) / 2
                    );
                }
              })
              .strength((d) =>
                that.isNetworkClusters ? 0.05 : d.type === "author" ? 0.2 : 10
              )
          )
          .force(
            "y",
      ...
      function initGraph() {
        this.doiToIndex = {};
        this.filteredAuthors = this.sessionStore.selectedPublicationsAuthors.slice(
          0,
        ...
        function initNodes() {
          ...
          if (this.showSuggestedNodes) {
            publications = publications.concat(
              this.sessionStore.suggestedPublicationsFiltered.slice(
                0,
                Math.round(this.suggestedNumberFactor * 50)
              )
            );
          }
        ...
        function initLinks() {
          ...
          if (this.showKeywordNodes) {
            this.sessionStore.uniqueBoostKeywords.forEach((keyword) => {
              this.sessionStore.publicationsFiltered.forEach((publication) => {
                if (
                  publication.doi in this.doiToIndex &&
          ...
          if (this.showAuthorNodes) {
            this.filteredAuthors.forEach((author) => {
              author.publicationDois.forEach((publicationDoi) => {
      ...
      function updateNodes() {
        ...
        try {
          ...
          throw new Error(
            "Cannot update author nodes in network: " + error.message
          );
        }
        function updatePublicationNodes() {
          const publicationNodes = this.node.filter(
          ...
          publicationNodes
            ...
            .classed(
              ...
              (d) =>
                ...
                !d.publication.isActive &&
                !d.publication.isLinkedToActive
            )
            ...
            .classed(
              "is-keyword-hovered",
              (d) => d.publication.isKeywordHovered
            )
        ...
        function updateKeywordNodes() {
          ...
          keywordNodes.attr(
            "data-tippy-content",
            (d) =>
              `Keyword <b>${d.id}</b> is matched in <b>${
                d.frequency
              }</b> publication${d.frequency > 1 ? "s" : ""}${
                this.sessionStore.isKeywordLinkedToActive(d.id)
        ...
        function updateAuthorNodes() {
          ...
          authorNodes
            .classed("linkedToActive", (d) =>
              d.author.publicationDois.includes(
          ...
          authorNodes.attr(
            "data-tippy-content",
            (d) => `<b>${d.author.name}</b> is linked
                        to <b>${d.author.count}</b> selected publication${
              d.author.count > 1 ? "s" : ""
            },
                        published ${
                        ...
                        with an aggregated, weighted score of <b>${
                          d.author.score
                        }</b>.
                    `
          );
      ...
      function updateYearLabels() {
        ...
        this.label
          ...
          .attr(
            "width",
            (d) => this.yearX(Math.min(d + 5, CURRENT_YEAR)) - this.yearX(d)
          )
          .attr("height", 20000)
