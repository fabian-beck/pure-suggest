...
export default class Publication {
    ...
    processData(data) {
        ...
        if (data.author) {
            ...
            this.author = data.author.replace(/(,\s+)(\d{4}-\d{4}-\d{4}-\d{3}[0-9Xx]{1})/g, "");
            this.authorOrcid = data.author;
            this.authorOrcidHtml = data.author.replace(/(,\s+)(\d{4}-\d{4}-\d{4}-\d{3}[0-9Xx]{1})/g, " <a href='https://orcid.org/$2'><img alt='ORCID logo' src='https://info.orcid.org/wp-content/uploads/2019/11/orcid_16x16.png' width='14' height='14' /></a>");
        }
        // container
        this.container = "";
