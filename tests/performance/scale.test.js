...
import { measureRenderTime, measureMemoryUsage, createMockPublications, PerformanceProfiler } from './utils.js';
...
describe('PublicationComponent Scale Performance Tests', () => {
  ...
  it('should handle 10 publications efficiently (baseline)', async () => {
    ...
    console.log(`✅ Small Scale (${SCALE_THRESHOLDS.small.count}): Render ${renderTime.toFixed(2)}ms, Avg Hover ${hoverMetrics.averageTime.toFixed(2)}ms`);
  });
  ...
  it('should handle 100 publications at medium scale', async () => {
    ...
    const hoverMetrics = await measureBatchOperations(sampleWrappers, 'hover-sample', async (wrapper) => {
      const el = wrapper.find('.publication-component');
      await el.trigger('mouseenter');
      await el.trigger('mouseleave');
    });
    ...
    console.log(`✅ Medium Scale (${SCALE_THRESHOLDS.medium.count}): Render ${renderTime.toFixed(2)}ms, Sample Hover ${hoverMetrics.averageTime.toFixed(2)}ms`);
  ...
  it('should handle 500 publications at large scale', async () => {
    ...
    const hoverMetrics = await measureBatchOperations(sampleWrappers, 'hover-large-sample', async (wrapper) => {
      const el = wrapper.find('.publication-component');
      await el.trigger('mouseenter');
    });
    ...
    const updateMetrics = await measureBatchOperations(sampleWrappers.slice(0, 5), 'reactive-updates', async (wrapper) => {
    ...
    console.log(`✅ Large Scale (${SCALE_THRESHOLDS.large.count}): Render ${renderTime.toFixed(2)}ms`);
    console.log(`📈 Hover Sample: ${hoverMetrics.averageTime.toFixed(2)}ms, Update Sample: ${updateMetrics.averageTime.toFixed(2)}ms`);
  });
  ...
  it('should handle 1000+ publications at extra large scale', async () => {
    ...
    const hoverMetrics = await measureBatchOperations(sampleWrappers, 'hover-xlarge-sample', async (wrapper) => {
      const el = wrapper.find('.publication-component');
      await el.trigger('mouseenter');
    });
    ...
    console.log(`✅ XLarge Scale (${SCALE_THRESHOLDS.xlarge.count}): Render ${renderTime.toFixed(2)}ms`);
  ...
  it('should identify performance regression patterns', async () => {
    ...
    for (const scale of scales) {
      ...
      const memoryAfter = measureMemoryUsage();
      const memoryUsedMB = memoryBefore && memoryAfter ? 
        (memoryAfter.used - memoryBefore.used) / 1024 / 1024 : 0;
