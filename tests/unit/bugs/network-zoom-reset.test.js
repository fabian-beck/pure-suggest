...
// Real d3 aliases its exported `zoomIdentity` with the object returned by
// `zoomTransform()` for any node that has not been zoomed/panned yet
// (d3-zoom/src/transform.js `defaultTransform`). Mock that same aliasing so
// a regression that mutates the transform returned by zoomTransform is caught.
const d3Module = createD3ModuleMock()
vi.mock('d3', () => ({ ...createD3ModuleMock(), zoomTransform: vi.fn(() => d3Module.zoomIdentity) }))
...
describe('Bug: reset zoom and pan only resets pan, not zoom', () => {
  let pinia

  beforeEach(() => {
    pinia = createPinia()
    setActivePinia(pinia)
    d3Module.zoomIdentity.k = 1
    d3Module.zoomIdentity.x = 0
    d3Module.zoomIdentity.y = 0
  ...
  it('does not mutate the shared d3.zoomIdentity transform when zooming in via the zoom-in button', () => {
    const sessionStore = useSessionStore()
    sessionStore.selectedPublications = [{ doi: '10.1234/test1', title: 'Test Publication' }]

    const wrapper = mount(NetworkVisComponent, {
      global: { plugins: [pinia], stubs: getComponentStubs() }
    })

    // Clicking "zoom in" before any prior pan/zoom gesture used to read the
    // shared zoomIdentity singleton (since the node has no zoom state yet)
    // and mutate its `k` in place, permanently corrupting what "reset" means.
    wrapper.vm.zoomByFactor(1.2)

    expect(d3Module.zoomIdentity.k).toBe(1)
  })
})
