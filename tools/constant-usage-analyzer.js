...
async function main() {
  ...
  console.log(`Found ${constantFiles.length} constant files:`, constantFiles.map(f => path.basename(f)).join(', '));
  
  // Get all source and test files to search through
  const allFiles = [
    ...(await getAllFiles(SRC_DIR)),
    ...(await getAllFiles(TESTS_DIR))
  ];
  ...
  if (singleUseConstants.length > 0) {
    ...
    singleUseConstants.forEach(result => {
      ...
      if (result.testFileCount > 0) {
        console.log(`    Also used in ${result.testFileCount} test file(s) (${result.testUsageCount} times) - not counted for decentralization`);
      }
    });
    console.log('');
  }
  ...
  if (multiUseConstants.length > 0) {
    ...
    multiUseConstants.forEach(result => {
      console.log(`  • ${result.constantName}: ${result.fileCount} files, ${result.usageCount} total uses`);
      result.usages.forEach(usage => {
        console.log(`    - ${path.relative(process.cwd(), usage.file)} (${usage.count}x)`);
      });
      if (result.testFileCount > 0) {
        console.log(`    + ${result.testFileCount} test file(s) (${result.testUsageCount} test uses)`);
      }
    });
  }
