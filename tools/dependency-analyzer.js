...
function analyzeFile(filePath) {
  try {
    ...
    if (dynamicImports) {
      const dynamicPaths = dynamicImports.map(match => {
    ...
    // Extract exports
    const exportMatches = content.match(/(export\s+(default\s+)?(function|class|const|let|var)\s+(\w+)|export\s*{\s*([^}]+)\s*})/g);
    if (exportMatches) {
    ...
    const functionMatches = content.match(/(?:function\s+(\w+)|const\s+(\w+)\s*=.*?(?:function|=>)|(\w+)\s*\([^)]*\)\s*{)/g);
    if (functionMatches) {
      ...
      }).filter(Boolean).slice(0, 5); // Limit to first 5 functions
    }
...
filesByType.forEach((files, type) => {
  ...
  files.forEach(([filePath, deps]) => {
    ...
    if (deps.imports && deps.imports.length > 0) {
      const localImports = deps.imports.filter(imp => imp.startsWith('./') || imp.startsWith('@/'));
      const externalImports = deps.imports.filter(imp => !imp.startsWith('./') && !imp.startsWith('@/'));
...
dependencies.forEach((deps, filePath) => {
  const connections = (deps.imports?.length || 0) + 
...
const sortedConnections = [...connectionCounts.entries()]
  .sort((a, b) => b[1] - a[1])
  .slice(0, 5);
